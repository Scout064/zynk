...
| `firewall` | all uOS based firewalls | USG FLEX 700H (V1.39) | `show config running \| no-pager` | pager additionally disabled session-wide via `cliconfig pager enabled false` | **Supported.** Upload the snapshot to `/conf/` trying **SFTP → SCP → FTP** in order (each on a dedicated connection; FTP requires the device's FTP server enabled — on failure the error lists the exact commands: `vrf main ftp-server enabled true` + `commit`), validate with `cmd config-apply option dry-run <file>` (checks the config WITHOUT applying), then `cmd config-apply <file>` — applies immediately **without a reboot**; success shows an `ok / message OK` response tree. A dry-run failure blocks the apply. A `copy-reboot` option exists (apply + reboot) but is not used. |
...
| Firewall (uOS) revert fails with `Garbage packet received` (in the SFTP error) | The firmware's SFTP subsystem doesn't work over the SSH session. Zynk automatically retries with SCP and then FTP — if all three fail, the error lists every attempt. Enable the device's FTP server (`vrf main ftp-server enabled true` + `commit`) so the FTP fallback can be used, and retry. |
